#!/usr/bin/env python3
"""
Temporal-Bootstrap-Monte-Carlo
==============================

Block-bootstraps minority-class windows, perturbs them with Gaussian noise,
and accumulates the resulting spatial density on a GeoTIFF.

Usage (single node, 8×A100):
    python temporal_bootstrap_mc.py \
...
Usage (SLURM, 256 GPUs):
    srun -N64 --ntasks-per-node=4 \
         python temporal_bootstrap_mc.py [args]
"""
...
import cupy as cp  # optional GPU path
...
def parse_args():
    ...
    p.add_argument("--output", required=True,
                   help="GeoTIFF path for density raster")
    ...
    p.add_argument("--block_len", type=int, default=30,
                   help="Length of bootstrap block (points)")
    p.add_argument("--sigma_time", type=float, default=0.5,
                   help="Std-dev (days) for temporal jitter")
    p.add_argument("--sigma_space", type=float, default=0.01,
                   help="Std-dev (degrees) for spatial jitter")
    p.add_argument("--gpu", action="store_true", help="Use CuPy for GPU")
    return p.parse_args()
...
class TemporalBootstrapMC:
    ...
    def run_mc(self):
        df = self.load_minority().compute()
        if len(df) < self.args.block_len:
            logging.warning("Dataset shorter than block length")
            return
        # MPI split
        ...
        for trial in range(local_trials):
            # 1. Random block start
            max_start = len(df) - self.args.block_len
            start = np.random.randint(0, max_start + 1)
            block = df.iloc[start:start + self.args.block_len].copy()

            # 2. Bootstrap: draw blocks with replacement
            n_blocks = max(1, int(np.ceil(len(df) / self.args.block_len)))
            boot_indices = np.random.randint(0, len(block), n_blocks)
            boot = block.iloc[np.concatenate(
                [np.arange(i, i + self.args.block_len) for i in boot_indices])]

            # 3. Add Gaussian jitter
            boot['timestamp'] += np.random.normal(0, self.args.sigma_time, len(boot))
            boot['lon'] += np.random.normal(0, self.args.sigma_space, len(boot))
            boot['lat'] += np.random.normal(0, self.args.sigma_space, len(boot))

            # 4. Accumulate density
            row, col = self.lonlat_to_rc(boot['lon'].values, boot['lat'].values)
...
def main():
    args = parse_args()
    tic = time.time()
    TemporalBootstrapMC(args).run_mc()
    if RANK == 0:
