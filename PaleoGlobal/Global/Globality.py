...
def calculate_globality_over_time(zircon_data, reconstruction_model, start_time, end_time, step, grid_size):
    """
    Calculate globality over a range of reconstruction times.
    
    Parameters:
    zircon_data (pandas.DataFrame): The zircon data.
    reconstruction_model: The reconstruction model (e.g., M2021).
    start_time (int): The starting reconstruction time in Ma.
    end_time (int): The ending reconstruction time in Ma.
    step (int): The step size for the reconstruction time in Ma.
    grid_size (int): The size of the grid in degrees.
    
    Returns:
    pandas.DataFrame: A DataFrame containing the reconstruction time and corresponding globality.
    """
    times = np.arange(start_time, end_time + 1, step)
    globalities = []

    for time in times:
        print(f"Calculating globality for {time} Ma...")
        globality = evaluate_globality_ancient(zircon_data, reconstruction_model, time, grid_size)
        globalities.append(globality)
...
# Load data
# csv_file = './data/u-pb_global_li.csv'  # Update the path to the CSV file
csv_file = 'C:/Users/Administrator/Desktop/global_u-pb_clean_cormodified_mass_rocktype_ages_main.csv'  # Update the path to the CSV file
...
# Fetch the reconstruction model
M2021 = Reconstructions.fetch_Merdith2021()

# Calculate globality over time
results_df = calculate_globality_over_time(zircon_data, M2021, start_time, end_time, step, grid_size)
